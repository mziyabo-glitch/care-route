-- BOOTSTRAP: Run this file in Supabase SQL Editor for a fresh database.
-- It includes ALL migrations including the payroll migration.
...
DECLARE r RECORD;
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'visits' AND table_type = 'BASE TABLE') THEN
    RETURN;
  END IF;
...
-- Care plans: client-linked plans and structured sections.
...
-- Visit-level care notes (multiple per visit).
